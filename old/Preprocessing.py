...
# Saves all outputs in a txt file
sys.stdout = open(dir+"preprocess_outputs.txt", "w")
name = "Abdo_US"  # MLMD use case (abdominal ultrasounds)

# Loading Data from CSV files
df_x_triage_input = pd.read_csv(
    '/triage.csv', encoding="latin-1")  # Triage Input Features
# CUI codes for triage notes free text
df_x_triage_CUI = pd.read_csv('/triage_CUI.csv')
df_labs = pd.read_csv('/ab.csv')  # Lab values for patients
idx_diagnosis = pd.read_csv('/idx.csv')  # Final Diagnosis for patients
triage_notes = pd.read_csv(
    '/notes_triage.csv', encoding="latin-1")  # Triage Input Features

# Processing Labs Data
lab_res = df.groupby('CSN').lab.apply(lambda x: pd.Series(
    {'Abdo_US': x.str.contains('Abdomen and Pelvis').any()})).reset_index()
...
tot['PrimaryDx_Label'] = tot['PrimaryDx'].apply(
    lambda x: 1 if 'ppendicitis' in x or 'varian' in x or 'ntussusception' in x or 'olvulus' in x else 0)
...
# Removes incomplete charts from our dataset (patients that did not have arrival times present)
tot = tot[tot.Arrived != False]
