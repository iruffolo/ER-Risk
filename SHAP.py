...
# From preprocessing file - this is the set of differential diagnoses used to
# act as the classification label
Label = 'PrimaryDx_Label'
# Upsampling of the minority class for training data only

# Note if you have a trained model already you can load your model rather than
# training a new model. This code demonstrates how to
...
def main():
    ...
    summary_plot = shap.summary_plot(
        shap_values[0], train_x, feature_names=feature, show=False)
    plt.savefig(path + 'NN_Shap_Summary_plot_3.png',
                bbox_inches='tight', dpi=600)
    plt.close()

    bar_plot = shap.summary_plot(
        shap_values, train_x, feature_names=feature, show=False)
    plt.savefig(path + 'NN_Shap_Bar_plot_3.png', bbox_inches='tight', dpi=600)
    plt.close()

    D_plot = shap.dependence_plot(
        "Sex_F", shap_values[0], train_x, interaction_index='Age', feature_names=feature)
    plt.savefig(path + 'Gender_NN_D_plot_3.png', bbox_inches='tight', dpi=600)

    Age_plot = shap.dependence_plot(
        "Age", shap_values[0], train_x, interaction_index=None, feature_names=feature)
    plt.savefig(path + 'Age_NN_D_plot_3.png', bbox_inches='tight', dpi=600)

    Pulse_plot = shap.dependence_plot(
        "Pulse", shap_values[0], train_x, interaction_index=None, feature_names=feature)
    plt.savefig(path + 'Pulse_NN_D_plot_3.png', bbox_inches='tight', dpi=600)

    Pulse_Age_plot = shap.dependence_plot(
        "Age", shap_values[0], train_x, interaction_index='Pulse', feature_names=feature)
